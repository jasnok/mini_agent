...
@agent_router.get("/health", tags=["01. LLM에서 Agent로"])
def health() -> dict:
    return {"status": "ok", "stage": "mini_agent_02_structured_output", "default_provider": settings.llm_provider}


@agent_router.get("/api/providers", tags=["01. LLM에서 Agent로"])
def providers() -> dict:
...
@agent_router.post(
    ...
    response_model=ConceptCompareResult,
    tags=["01. LLM에서 Agent로"],
)
...
@agent_router.post(
    "/api/travel/classify",
    response_model=TravelIntentResult,
    tags=["01. LLM에서 Agent로"],
)
...
@agent_router.post(
    ...
    response_model=ProviderCompareResult,
    tags=["01. LLM에서 Agent로"],
)
