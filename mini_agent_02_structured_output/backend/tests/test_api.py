...
def test_openapi_exposes_generic_structured_route_only() -> None:
    ...
    assert paths["/api/generate"]["post"]["tags"] == ["01. LLM에서 Agent로"]
    assert paths["/api/media/tts"]["post"]["tags"] == ["01. LLM에서 Agent로"]
    assert paths["/api/prompts/preview"]["post"]["tags"] == [
        "02. Prompt와 구조화 출력"
    ]
    assert paths["/api/structured/generate"]["post"]["tags"] == [
        "02. Prompt와 구조화 출력"
    ]
